...
> **Files:** `18_WorkloadTracker.gs` (standalone portal), `25_WorkloadService.gs` (SPA module) | **Version:** 4.10.0 / 4.13.0

### Standalone Portal (v4.10.0)

| Feature | Description | Function | Keywords |
|---------|-------------|----------|----------|
| **Submit Workload Data** | Members submit weekly caseload data via PIN-auth portal | `processWorkloadForm(data)` | submit, caseload, weekly |
| **Workload Dashboard** | View collective anonymized stats with reciprocity enforcement | `getWorkloadDashboardData(memberId)` | dashboard, stats, anonymous |
| **User History** | View personal submission history | `getWorkloadUserHistory(memberId)` | history, submissions, personal |
| **Export History** | Export personal submission history | `exportWorkloadHistoryPortal(memberId)` | export, download, history |
