...
> ⚠️ **IMPORTANT: Deploy ONLY `dist/ConsolidatedDashboard.gs`**
> Run `node build.js` to generate the consolidated file from the 9 modular source files.
> Deploying individual source files will cause function conflicts.

1. Run `node build.js` to generate `dist/ConsolidatedDashboard.gs`
2. Copy the consolidated file to your Google Apps Script project
3. Run `CREATE_509_DASHBOARD()` to create 11 sheets + 6 hidden calculation sheets
...
**Location:** `04_UIService.gs` (Mobile UI functions merged here)
...
This repository implements a **9-file modular architecture** following the Separation of Concerns principle. Files are numbered for proper dependency order and GitHub grouping.
...
├── dist/                       # Build output (generated by node build.js)
...
**08_Code.gs** (~11,215 lines) - Core setup, hidden sheets, formulas, dashboard creation
...
**07_DevTools.gs** (~2,081 lines) - DELETE THIS FILE BEFORE PRODUCTION
...
**08_Code.gs - Hidden Sheets Section** (merged from HiddenSheets.gs + FormulaService.gs)
...
**06_Maintenance.gs - Data Integrity Section** (merged from DataIntegrity.gs + PerformanceUndo.gs)
...
**06_Maintenance.gs - Performance/Undo Section** (merged into Maintenance)
- Caching:
...
**04_UIService.gs - Mobile Actions Section** (merged into UIService)
- Mobile Interface:
...
**05_Integrations.gs - WebApp Section** (merged into Integrations)
- Web App Entry Point:
...
**08_Code.gs - Formula Service Section** (merged into Code)
- `setupCalcMembersSheet()` - Setup member statistics sheet
- `setupCalcGrievancesSheet()` - Setup grievance aggregations sheet
- `setupCalcDeadlinesSheet()` - Setup deadline calculations sheet
- `setupCalcStatsSheet()` - Setup dashboard statistics sheet
- `setupCalcSyncSheet()` - Setup cross-sheet sync sheet
- `setupCalcFormulasSheet()` - Setup formula references sheet
- `getCalcValue()` - Get value from hidden calculation sheet
- `getDashboardStats()` - Get aggregated dashboard statistics
- `searchDashboard()` - Full-text search across sheets
...
**03_GrievanceManager.gs** (~1,048 lines) - Grievance Lifecycle Management
...
**05_Integrations.gs** (~2,098 lines) - Drive, Calendar, WebApp, Notifications
- Google Drive Integration:
...
**06_Maintenance.gs** (~2,521 lines) - Admin, Diagnostics, Data Integrity, Caching
- `DIAGNOSE_SETUP()` - System health check
- `REPAIR_DASHBOARD()` - Repair hidden sheets and triggers
...
**09_Main.gs** (~767 lines) - Entry Point & Triggers
