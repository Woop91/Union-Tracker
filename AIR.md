...
This repository implements a **modular multi-file architecture** following the Separation of Concerns principle. Each module handles a specific aspect of the dashboard functionality.
...
│   ├── PerformanceUndo.gs      # Caching layer & undo/redo system (50 action history)
│   ├── ComfortViewFeatures.gs  # Comfort View accessibility & theming (focus mode, pomodoro)
│   ├── HiddenSheets.gs         # Self-healing hidden calculation sheets with auto-sync
│   ├── FormulaService.gs       # Hidden sheet setup helpers and search functions
│   ├── UIService.gs            # UI components, dialogs, sidebars
│   ├── GrievanceManager.gs     # Grievance lifecycle, deadlines, step advancement
│   ├── Integrations.gs         # Google Drive, Calendar, email notifications
│   ├── DataIntegrity.gs        # Batch operations, validation, orphan detection, archiving
│   ├── Maintenance.gs          # Admin tools, diagnostics (DIAGNOSE_SETUP, REPAIR_DASHBOARD)
...
│   ├── Code.gs                 # Core setup, forms, dashboard creation, multi-select
│   ├── Main.gs                 # Entry point, onOpen, onEdit triggers
...
**FormulaService.gs** (~744 lines) - Hidden Sheet Setup Helpers
...
**UIService.gs** (~1237 lines) - UI Components & Dialogs
...
**Maintenance.gs** (~745 lines) - Admin & Diagnostic Tools
- `DIAGNOSE_SETUP()` - System health check
- `REPAIR_DASHBOARD()` - Repair hidden sheets and triggers
