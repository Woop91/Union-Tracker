...
// ============================================================================
// SCHEDULED HEALTH CHECK (v4.37.0)
// ============================================================================

/**
 * Automated health check designed to run on a daily time-driven trigger.
 * Runs DIAGNOSE_SETUP(), auto-heals common issues (missing sheets, hidden-sheet
 * enforcement), and emails the admin on non-healable errors.
 *
 * Unlike DIAGNOSE_SETUP() which is manual and reports to the caller, this
 * function is autonomous: it acts on what it finds and only alerts when it
 * can't fix things itself.
 *
 * Install trigger: installScheduledHealthCheckTrigger()
 * @returns {Object} { status, autoHealed, alertSent, issues }
 */
function scheduledHealthCheck() {
  var result = { status: 'OK', autoHealed: [], alertSent: false, issues: [] };

  try {
    // Run diagnostics
    var diagnosis = DIAGNOSE_SETUP();
    result.issues = (diagnosis.errors || []).concat(diagnosis.warnings || []);

    // Auto-heal: recreate missing sheets
    var missingSheetErrors = diagnosis.errors.filter(function(e) {
      return e.indexOf('Missing required sheet') !== -1 || e.indexOf('Missing hidden sheet') !== -1;
    });
    if (missingSheetErrors.length > 0) {
      try {
        if (typeof _ensureAllSheetsInternal === 'function') {
          _ensureAllSheetsInternal();
          result.autoHealed.push('Recreated missing sheets (' + missingSheetErrors.length + ')');
        }
      } catch (healErr) {
        result.issues.push('Auto-heal failed: ' + healErr.message);
      }
    }

    // Auto-heal: enforce hidden sheets
    try {
      var enforceResult = enforceHiddenSheets();
      if (enforceResult && enforceResult.enforced > 0) {
        result.autoHealed.push('Re-hid ' + enforceResult.enforced + ' sheets');
      }
    } catch (hideErr) {
      result.issues.push('Hidden sheet enforcement failed: ' + hideErr.message);
    }

    // Monitor PropertiesService quota
    try {
      var props = PropertiesService.getScriptProperties();
      var allProps = props.getProperties();
      var totalSize = JSON.stringify(allProps).length;
      if (totalSize > 400000) { // 80% of 500KB limit
        result.issues.push('ScriptProperties at ' + Math.round(totalSize / 1024) + 'KB / 500KB (80%+ usage)');
        if (typeof recordSecurityEvent === 'function') {
          recordSecurityEvent('QUOTA_WARNING', 'MEDIUM',
            'ScriptProperties at ' + Math.round(totalSize / 1024) + 'KB/500KB');
        }
      }
    } catch (_propErr) {
      // Non-critical — skip quota check
    }

    // Determine final status
    var nonHealedErrors = diagnosis.errors.filter(function(e) {
      return missingSheetErrors.indexOf(e) === -1;
    });

    if (nonHealedErrors.length > 0) {
      result.status = 'ERROR';
      // Alert admin on non-healable errors
      try {
        var adminEmail = Session.getActiveUser().getEmail();
        if (adminEmail && typeof safeSendEmail_ === 'function') {
          var version = (typeof COMMAND_CONFIG !== 'undefined') ? COMMAND_CONFIG.VERSION : 'unknown';
          safeSendEmail_({
            to: adminEmail,
            subject: '[Health Check] ' + nonHealedErrors.length + ' issue(s) require attention',
            body: 'Automated health check found issues that could not be auto-healed:\n\n' +
                  nonHealedErrors.join('\n') +
                  '\n\nAuto-healed: ' + (result.autoHealed.length > 0 ? result.autoHealed.join(', ') : 'none') +
                  '\nVersion: ' + version +
                  '\nTimestamp: ' + new Date().toISOString()
          });
        ...
        Logger.log('scheduledHealthCheck: failed to send alert: ' + emailErr.message);
      }
    ...
    if (typeof logAuditEvent === 'function') {
      logAuditEvent(AUDIT_EVENTS.SYSTEM_REPAIR, {
        ...
        autoHealed: result.autoHealed.length,
        issues: result.issues.length
      });
    }
  } catch (fatalErr) {
    Logger.log('scheduledHealthCheck FATAL: ' + fatalErr.message);
    result.status = 'FATAL';
    result.issues.push(fatalErr.message);
  }

  Logger.log('scheduledHealthCheck: ' + result.status +
    ' | healed=' + result.autoHealed.length +
    ' | issues=' + result.issues.length +
...
/**
 * Installs a daily time-driven trigger for the scheduled health check.
 * Safe to call multiple times — skips if trigger already exists.
 */
function installScheduledHealthCheckTrigger() {
  var ss = SpreadsheetApp.getActiveSpreadsheet();
  var triggers = ScriptApp.getUserTriggers(ss);

  for (var i = 0; i < triggers.length; i++) {
    if (triggers[i].getHandlerFunction() === 'scheduledHealthCheck') {
      Logger.log('Scheduled health check trigger already installed');
      return;
    }
  }
  ...
  ScriptApp.newTrigger('scheduledHealthCheck')
    .timeBased()
    .everyDays(1)
    .atHour(6) // 6 AM ET — before users arrive
    .create();

  Logger.log('Installed daily scheduled health check trigger (6 AM)');
}
