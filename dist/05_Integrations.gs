 ...
 * Routes admin/steward sheet-bound pages to _legacyDoGet() and all other
 * requests to the new web-dashboard SPA via doGetWebDashboard().
...
function doGet(e) {
  ...
  // Default: new web-dashboard SPA (SSO + magic link auth, Mono Signal / Glass Depth themes)
  return doGetWebDashboard(e);
}

/**
 * Legacy doGet handler — preserved for backward-compatible admin routes.
 * Previously the primary doGet; now called only for ?page=search|grievances|members|links
 * or ?mode=steward|member.
 */
