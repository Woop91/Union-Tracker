 ...
 * @private
 * @returns {string} Organization name (e.g., "Your Union Local")
 */
...
var VERSION_INFO = (function() {
  var ver = (typeof COMMAND_CONFIG !== 'undefined' && COMMAND_CONFIG.VERSION) ? COMMAND_CONFIG.VERSION : '4.34.3';
  var parts = ver.split('.');
  return {
    version: ver,
    MAJOR: parts.length > 0 ? parseInt(parts[0], 10) : 4,
    MINOR: parts.length > 1 ? parseInt(parts[1], 10) : 34,
    PATCH: parts.length > 2 ? parseInt(parts[2], 10) : 3,
    BUILD: 'v' + ver,
    CURRENT: ver,
...
var VERSION_HISTORY = [
  ...
  { version: '4.32.0', date: '2026-03-19', codename: 'Workforce Mobility Survey', changes: 'New survey Section 13 (WORKFORCE_RETENTION) and Section 13A (WORKFORCE_LEAVING) added to quarterly member survey. Section 13 (always shown, 4 questions): q80 likelihood to stay (radio: Very Likely–Very Unlikely), q81 exploring outside the organization (radio-branch — Yes triggers 13A), q84 union addressing retention factors (slider-10), q86 optional open text. Section 13A (conditional on q81=Yes, 2 questions): q82 types of opportunities outside the organization (checkbox, max 2: state transfer | leaving state service | private | non-profit/education | not sure), q83 reasons for leaving (checkbox, max 3: Pay & Benefits | Workload | Management | Limited Advancement or Transfer Opportunities | Work-Life Balance | RTO Policy | Burnout | Culture | Other — transfer awareness from former q85 folded into this option). Standalone q85 removed. Looker: Workforce Retention Avg, Likelihood to Stay, Exploring Outside columns added to both _Looker_Satisfaction and _Looker_Anon_Satisfaction headers and refresh functions. getSatisfactionSummary() workforce section added. Section colors: WORKFORCE_RETENTION #e8f4f8, WORKFORCE_LEAVING #fdecea. Always active (not toggled). Visible to all dashboard roles.' },
...
var SHEETS = {
  ...
  PORTAL_EVENTS:       'Events',              // visible — union events calendar entries
  PORTAL_MINUTES:      'MeetingMinutes',      // visible — meeting minutes archive
 ...
 *
 * SOURCE: Based on standard union CBA grievance procedures.
 * Actual deadlines are configurable via the Config sheet.
 *
