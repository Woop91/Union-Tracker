 ...
 * Example: "Your Union Local" → "Your Union Local Dashboard"
...
var VERSION_INFO = (function() {
  var ver = (typeof COMMAND_CONFIG !== 'undefined' && COMMAND_CONFIG.VERSION) ? COMMAND_CONFIG.VERSION : '4.43.1';
  var parts = ver.split('.');
  return {
    version: ver,
    MAJOR: parts.length > 0 ? parseInt(parts[0], 10) : 4,
    MINOR: parts.length > 1 ? parseInt(parts[1], 10) : 43,
    PATCH: parts.length > 2 ? parseInt(parts[2], 10) : 1,
    BUILD: 'v' + ver,
    CURRENT: ver,
    BUILD_DATE: '2026-03-27',
    CODENAME: 'One-Tap Check-In',
    codename: 'One-Tap Check-In'
  };
})();
...
var VERSION_HISTORY = [
  ...
  { version: '4.32.0', date: '2026-03-19', codename: 'Workforce Mobility Survey', changes: 'New survey Section 13 (WORKFORCE_RETENTION) and Section 13A (WORKFORCE_LEAVING) added to quarterly member survey. Section 13 (always shown, 4 questions): q80 likelihood to stay (radio: Very Likely–Very Unlikely), q81 exploring outside your organization (radio-branch — Yes triggers 13A), q84 union addressing retention factors (slider-10), q86 optional open text. Section 13A (conditional on q81=Yes, 2 questions): q82 types of opportunities outside your organization (checkbox, max 2: MA state transfer | leaving state service | private | non-profit/education | not sure), q83 reasons for leaving (checkbox, max 3: Pay & Benefits | Workload | Management | Limited Advancement or Transfer Opportunities | Work-Life Balance | RTO Policy | Burnout | Culture | Other — transfer awareness from former q85 folded into this option). Standalone q85 removed. Looker: Workforce Retention Avg, Likelihood to Stay, Exploring Outside Options columns added to both _Looker_Satisfaction and _Looker_Anon_Satisfaction headers and refresh functions. getSatisfactionSummary() workforce section added. Section colors: WORKFORCE_RETENTION #e8f4f8, WORKFORCE_LEAVING #fdecea. Always active (not toggled). Visible to all dashboard roles.' },
 ...
 *
 * SOURCE: Your Union Local Unit 8 CBA (2024–2026), Article 23 / MA DOC Grievance
