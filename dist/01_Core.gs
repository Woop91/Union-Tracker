 ...
 * - Public data* endpoints (called via google.script.run): use successResponse()/errorResponse()
 * - Internal helpers: may throw or return null (callers handle)
 * - Menu-triggered functions: use showToast/showAlert for user feedback
 *
 * When adding new endpoints, prefer successResponse()/errorResponse() for consistency.
 * Do NOT refactor existing return types without updating all callers.
 */
  ...
  try {
    ...
    if (!sheet) {
      try {
        ...
        // Race condition: another execution may have created the sheet concurrently
        sheet = ss.getSheetByName(ERROR_CONFIG.LOG_SHEET_NAME);
        if (!sheet) return; // truly failed
      }
    }
...
function _readConfigCell(cacheKey, colConst, fallback) {
  ...
  try {
    ...
    if (configSheet && colConst) {
      var raw = configSheet.getRange(3, colConst).getValue();
      var val = (raw != null && raw !== '') ? String(raw).trim() : '';
      if (val !== '') {
        _configCellCache[cacheKey] = val;
        return _configCellCache[cacheKey];
      }
    }
  } catch (_e) { log_('_readConfigCell', 'Error: ' + (_e.message || _e)); }
...
function getVersionHistory_() {
  ...
  { version: '4.45.0', date: '2026-03-28', codename: 'PIN hardening', changes: 'generateMemberPIN() uses Utilities.getUuid() rejection sampling for uniform 6-digit PINs. (An earlier plan to derive PINs from member phone numbers was not shipped — production code is fully random.) PIN sessions blocked from poll submissions (wqSubmitPoolQuestion). Theme cleanup: removed 5 novelty themes (Comic, Brutalist, Retro OS, Liquid Pour, Blob Lava). Removed Lao Tzu duplicate quote from Knowledge Engine.' },
  ...
  { version: '4.36.0', date: '2026-03-24', codename: 'Centralized Modal Hub', changes: 'Modal Hub planned but superseded by Tab Modals (v4.48.0, ENABLE_TAB_MODALS + TAB_MODAL_REGISTRY). Tab review XSS/error handling fixes: DataService try/catch, AUDIT_LOG_COLS constants, poms_reference pesc() XSS prevention, grievance_form failureHandler + email injection guard.' },
...
var MEMBER_COLS = buildColsFromMap_(MEMBER_HEADER_MAP_, {
  ...
  DIRECTOR: 'MANAGER'         // header is "Director"; key kept as MANAGER for backward compat
});
...
var CONFIG_HEADER_MAP_ = [
  ...
  { key: 'MANAGERS',              header: 'Directors',                         type: 'list' },
  ...
  { key: 'CALENDAR_ID',           header: 'Google Calendar ID',                type: 'calid' },
...
var CONFIG_DATA_VALIDATORS = {
  ...
  calid:   function(v) { return /^[a-zA-Z0-9._@-]{5,}$/.test(String(v).trim()); }
};
...
function _isGrievancesEnabled() {
  try {
    ...
    // Fail-open is intentional — default to showing grievances — but log the
    // real error so a silent config-read failure doesn't stay hidden.
    if (typeof log_ === 'function') log_('_isGrievancesEnabled', 'config read failed: ' + (_e.message || _e));
    return true;
  }
}
...
/**
 * Resolve a single column position by reading actual sheet headers at runtime.
 * Returns the 1-indexed column position of headerName in row 1 of the sheet,
 * or fallback if the header is not found (or the sheet is null/empty/errored).
 * @param {Sheet|null} sheet - The sheet to read headers from
 * @param {string} headerName - The header text to search for
 * @param {number} [fallback] - Value to return if not found (default: 1)
 * @returns {number} 1-indexed column position
 */
function resolveColumnByHeader_(sheet, headerName, fallback) {
  var fb = (fallback !== undefined && fallback !== null) ? fallback : 1;
  try {
    if (!sheet) return fb;
    var lastCol = sheet.getLastColumn();
    if (lastCol < 1) return fb;
    var headers = sheet.getRange(1, 1, 1, lastCol).getValues()[0];
    var needle = String(headerName).trim();
    for (var i = 0; i < headers.length; i++) {
      if (String(headers[i]).trim() === needle) return i + 1;
    }
    return fb;
  } catch (_e) {
    return fb;
...
/**
 * Resolve multiple column positions in a single call by reading actual sheet
 * headers once. Each entry specifies a key, header name, and fallback.
 * @param {Sheet|null} sheet - The sheet to read headers from
 * @param {Array<{key: string, header: string, fallback: number}>} entries
 * @returns {Object} Map of key → 1-indexed column position
 */
function resolveColumnsByHeader_(sheet, entries) {
  var result = {};
  var headers = null;
  try {
    if (sheet) {
  ...
  for (var e = 0; e < entries.length; e++) {
    var entry = entries[e];
    var fb = (entry.fallback !== undefined && entry.fallback !== null) ? entry.fallback : 1;
    if (!headers) { result[entry.key] = fb; continue; }
    var needle = String(entry.header).trim();
    var found = false;
    for (var i = 0; i < headers.length; i++) {
      if (String(headers[i]).trim() === needle) {
        result[entry.key] = i + 1;
        found = true;
        break;
      }
    }
    if (!found) result[entry.key] = fb;
  }
  return result;
}
...
function syncColumnMaps() {
  ...
  var maps = [
    { name: 'MEMBER_COLS', sheet: SHEETS.MEMBER_DIR, map: MEMBER_HEADER_MAP_, target: MEMBER_COLS,
      ...
      headerRenames: { 'Manager': 'Director' } },
    ...
    { name: 'CONFIG_COLS', sheet: SHEETS.CONFIG, map: CONFIG_HEADER_MAP_, target: CONFIG_COLS,
      opts: { headerRow: 2 },
      headerRenames: { 'Managers': 'Directors' } },
  ...
  for (var m = 0; m < maps.length; m++) {
    ...
    if (sheet && sheet.getLastColumn() > 0) {
      try {
        ...
        // Rename old headers in-place BEFORE checking for missing columns.
        // Without this, a renamed header (e.g. "Manager"→"Director") would be
        // seen as missing, causing a duplicate column to be appended.
        if (entry.headerRenames) {
          for (var ri = 0; ri < sheetHeaders.length; ri++) {
            var oldH = String(sheetHeaders[ri]).trim();
    ...
    for (var key in resolved) {
      if (Object.prototype.hasOwnProperty.call(resolved, key) && entry.target[key] !== resolved[key]) {
    ...
    if (entry.aliases) {
      for (var alias in entry.aliases) {
        if (Object.prototype.hasOwnProperty.call(entry.aliases, alias)) {
...
function persistColumnMaps_() {
  try {
    var data = {
      ...
      // Ensure syncColumnMaps() resolutions for these sheets persist across
      // execution contexts too; otherwise onEdit/doGet load stale positions.
      RESOURCES_COLS: (typeof RESOURCES_COLS !== 'undefined') ? RESOURCES_COLS : undefined,
      RESOURCE_CONFIG_COLS: (typeof RESOURCE_CONFIG_COLS !== 'undefined') ? RESOURCE_CONFIG_COLS : undefined
    };
...
function loadCachedColumnMaps_() {
  try {
    ...
    if (typeof RESOURCES_COLS !== 'undefined') targets.RESOURCES_COLS = RESOURCES_COLS;
    if (typeof RESOURCE_CONFIG_COLS !== 'undefined') targets.RESOURCE_CONFIG_COLS = RESOURCE_CONFIG_COLS;

    var changed = false;
    for (var name in targets) {
      if (data[name]) {
        for (var key in data[name]) {
          if (Object.prototype.hasOwnProperty.call(data[name], key) && Object.prototype.hasOwnProperty.call(targets[name], key)
...
var GRIEVANCE_STATUS = {
  DRAFT: 'Draft',                     // v4.55.1 H07-BUG-05: used by DataService.startGrievanceDraft
  OPEN: 'Open',
...
function getDeadlineRules() {
  try {
    ...
    if (configSheet) {
      ...
      // parseFloat returns NaN for "" so isNaN() fallbacks fire; Number("") === 0 bypassed the fallbacks.
      var filing     = parseFloat(dVals[0]);
      var s1Resp     = parseFloat(dVals[CONFIG_COLS.STEP1_RESPONSE_DAYS - colStart]);
      var s2Appeal   = parseFloat(dVals[CONFIG_COLS.STEP2_APPEAL_DAYS   - colStart]);
      var s2Resp     = parseFloat(dVals[CONFIG_COLS.STEP2_RESPONSE_DAYS - colStart]);
      var s3Appeal   = parseFloat(dVals[CONFIG_COLS.STEP3_APPEAL_DAYS   - colStart]);
      var s3Resp     = parseFloat(dVals[CONFIG_COLS.STEP3_RESPONSE_DAYS - colStart]);
      var arbDemand  = parseFloat(dVals[CONFIG_COLS.ARBITRATION_DEMAND_DAYS - colStart]);
      return {
    ...
    log_('getDeadlineRules', 'Error reading deadline config: ' + e.message);
  }
...
var AUDIT_EVENTS = {
  ...
  // v4.55.1 H07-BUG-02: Constant previously referenced by 05_Integrations.gs via `||` fallback
  GRIEVANCE_SIGNED: 'GRIEVANCE_SIGNED',
...
var BATCH_LIMITS = {
  ...
  EXECUTION_BUFFER_MS: 30000,         // 30s safety buffer before GAS timeout
...
} catch (_initCache) { try { log_('loadCachedColumnMaps_', 'Column map cold-cache init failed: ' + (_initCache.message || _initCache)); } catch(_) {} }
