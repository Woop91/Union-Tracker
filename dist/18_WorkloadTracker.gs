...
// Dead code removed: initWorkloadTrackerSheets — now in 25_WorkloadService.gs
  ...
  try {
    ...
    return withLock(function() {
      ...
      vault.appendRow([
        ...
        escapeForFormula(sanitizeString(formObj.employment_type || 'Full-time', 20)),  // Employment Type
        formObj.employment_type === 'Part-time' ? (Number(formObj.part_time_hours) || '') : '',
        escapeForFormula(sanitizeString(formObj.leave_type || '', 20)),              // Leave Type
        escapeForFormula(sanitizeString(formObj.leave_planned || '', 20)),           // Leave Planned
        leaveStart,                                                                  // Leave Start
        leaveEnd,                                                                    // Leave End
        escapeForFormula(sanitizeString(formObj.no_intake_choice || '', 20)),        // No Intake Choice
        escapeForFormula(sanitizeString(formObj.notice_time || '', 20)),             // Notice Time
        formObj.half_day_leave ? 'Yes' : '',                                         // Half Day
        escapeForFormula(sanitizeString(formObj.privacy || 'Unit', 20)),             // Privacy
...
function wtSaveReminderPreference_(prefs) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (data[i][WT_REMINDERS_COLS.EMAIL] &&
      ...
      // CR-FORMULA: Escape user-supplied preference strings
      sheet.getRange(i + 1, 1, 1, 5).setValues([[
        emailLower, prefs.enabled ? 'Yes' : 'No',
        escapeForFormula(prefs.frequency), escapeForFormula(prefs.day), escapeForFormula(prefs.time)
      ]]);
      return;
    }
  }
  sheet.appendRow([emailLower, prefs.enabled ? 'Yes' : 'No', escapeForFormula(prefs.frequency), escapeForFormula(prefs.day), escapeForFormula(prefs.time), '']);
}
...
// Dead code removed: refreshWorkloadLedger — now in 25_WorkloadService.gs
...
// Dead code removed: createWorkloadBackup — now in 25_WorkloadService.gs
...
// Dead code removed: showWorkloadHealthStatus, setupWorkloadReminderSystem,
// processWorkloadReminders — now in 25_WorkloadService.gs
...
// Dead code removed: SSO wrappers (processWorkloadFormSSO, getWorkloadHistorySSO,
// getWorkloadDashboardDataSSO, getWorkloadReminderSSO, setWorkloadReminderSSO)
// now live in 25_WorkloadService.gs as thin wrappers around WorkloadService IIFE.
