...
async function renderResearch(page){
  ...
  try{
    ...
    var membership=result.statistics.filter(function(row){return row.topic==="union_membership_rate"}).sort(function(a,b){return String(a.period).localeCompare(String(b.period))});
...
if(EMBEDDED){
  ...
  initPromise=rpc("init-url",{packages:{directory:INDEX.packages.directory.path,evidence:INDEX.packages.evidence.path}});
}
