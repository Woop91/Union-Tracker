...
// ============================================================================
// STALE DATA DETECTION (v4.37.0)
// ============================================================================

/**
 * Validates that a sheet's headers match expected columns and that the sheet
 * contains data rows. Catches silent data corruption (deleted headers, broken
 * formulas, accidentally cleared sheets) at read time instead of letting stale
 * data propagate through the system.
 *
 * Returns { valid: true } on success, or { valid: false, issues: [...] } on
 * failure. Callers decide how to respond (log warning, degrade gracefully, etc.).
 *
 * @param {Sheet} sheet - The sheet to validate
 * @param {string[]} requiredHeaders - Header labels that must be present (case-insensitive)
 * @param {Object} [opts] - Options
 * @param {number} [opts.minRows=0] - Minimum expected data rows (excluding header)
 * @param {boolean} [opts.logWarnings=true] - Whether to log issues via handleError
 * @returns {{ valid: boolean, issues: string[], headerRow: string[] }}
 */
function validateSheetData_(sheet, requiredHeaders, opts) {
  opts = opts || {};
  var logWarnings = opts.logWarnings !== false;
  var minRows = opts.minRows || 0;
  var result = { valid: true, issues: [], headerRow: [] };

  if (!sheet) {
    result.valid = false;
    result.issues.push('Sheet is null');
    return result;
  }
  ...
  if (lastRow === 0 || lastCol === 0) {
    result.valid = false;
    result.issues.push(sheetName + ': sheet is completely empty (0 rows or 0 columns)');
    if (logWarnings && typeof handleError === 'function') {
      handleError(new Error(result.issues[0]), 'staleDataDetection', 'WARNING');
    }
    return result;
  }
  ...
  result.headerRow = headers.map(function(h) { return String(h || '').trim(); });
  var lowerHeaders = result.headerRow.map(function(h) { return h.toLowerCase(); });

  // Check required headers
  if (requiredHeaders && requiredHeaders.length > 0) {
    var missing = [];
    for (var i = 0; i < requiredHeaders.length; i++) {
      var needle = requiredHeaders[i].toLowerCase();
      if (lowerHeaders.indexOf(needle) === -1) {
        missing.push(requiredHeaders[i]);
      }
    }
    if (missing.length > 0) {
      result.valid = false;
      result.issues.push(sheetName + ': missing required headers: ' + missing.join(', '));
    }
  }
  ...
  if (minRows > 0 && dataRows < minRows) {
    result.valid = false;
    result.issues.push(sheetName + ': expected at least ' + minRows + ' data rows, found ' + dataRows);
  }

  // Log issues
  if (!result.valid && logWarnings && typeof handleError === 'function') {
    for (var j = 0; j < result.issues.length; j++) {
      handleError(new Error(result.issues[j]), 'staleDataDetection', 'WARNING');
    }
  }
