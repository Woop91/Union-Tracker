...
/**
 * Verifies all hidden sheets exist and are properly configured
...
function verifyHiddenSheets() {
  ...
  for (const [key, sheetName] of Object.entries(HIDDEN_SHEETS)) {
    const sheet = ss.getSheetByName(sheetName);

    if (!sheet) {
      results.missing.push(sheetName);
    } else {
      // Check if sheet has expected formulas
      const hasFormulas = checkSheetFormulas(sheet, key);
      if (hasFormulas) {
        results.verified.push(sheetName);
      } else {
        results.corrupted.push(sheetName);
      }
    }
  }
...
/**
 * Checks if a hidden sheet has its required formulas
 * @param {Sheet} sheet - The sheet to check
 * @param {string} sheetKey - The key from HIDDEN_SHEETS
 * @return {boolean} True if formulas are present
 */
function checkSheetFormulas(sheet, sheetKey) {
  try {
    // Each hidden sheet should have at least some formulas in cell A2
 ...
 * @return {Object} Result with count of fixes
 */
function fixDataQualityIssues() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  let fixCount = 0;

  // Fix 1: Trim whitespace in Member Directory
  const memberSheet = ss.getSheetByName(SHEET_NAMES.MEMBER_DIRECTORY);
  if (memberSheet) {
    ...
    for (let i = 1; i < data.length; i++) {
      for (const col of textColumns) {
        ...
        if (typeof value === 'string') {
          ...
          if (trimmed !== value) {
            memberSheet.getRange(i + 1, col + 1).setValue(trimmed);
            fixCount++;
          }
        }
      }
  ...
  // Fix 2: Standardize grievance status values
  const grievanceSheet = ss.getSheetByName(SHEET_NAMES.GRIEVANCE_TRACKER);
  if (grievanceSheet) {
    const data = grievanceSheet.getDataRange().getValues();
    const validStatuses = Object.values(GRIEVANCE_STATUS);

    for (let i = 1; i < data.length; i++) {
      const status = data[i][GRIEVANCE_COLUMNS.STATUS];
      if (status && !validStatuses.includes(status)) {
        // Try to match to valid status
        const matched = validStatuses.find(s =>
          s.toLowerCase() === status.toString().toLowerCase()
        );
        if (matched) {
          grievanceSheet.getRange(i + 1, GRIEVANCE_COLUMNS.STATUS + 1).setValue(matched);
          fixCount++;
        }
      }
    }
  ...
  // Fix 3: Update missing Last Updated timestamps
  if (grievanceSheet) {
    const data = grievanceSheet.getDataRange().getValues();
    const now = new Date();
    ...
    for (let i = 1; i < data.length; i++) {
      ...
      const hasData = data[i][GRIEVANCE_COLUMNS.GRIEVANCE_ID];

      if (hasData && !lastUpdated) {
        grievanceSheet.getRange(i + 1, GRIEVANCE_COLUMNS.LAST_UPDATED + 1).setValue(now);
        fixCount++;
      }
    }
  }
