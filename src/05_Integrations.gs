...
function _getOrCreateNamedFolder_(name, propKey, props, configSheet, parentFolder) {
  ...
  if (storedId) {
    ...
    } catch (_e) { log_('_getOrCreateNamedFolder_', 'Error: ' + (_e.message || _e)); }
  }
...
function setupDashboardCalendar() {
  ...
  if (storedId) {
    ...
    } catch (_e) { log_('setupDashboardCalendar', 'Error: ' + (_e.message || _e)); }
  }
...
function getOrCreateRootFolder() {
  ...
  if (storedId) {
    ...
    } catch (_e) { log_('getOrCreateRootFolder', 'Error: ' + (_e.message || _e)); }
  }
  ...
  try {
    rootFolder.setSharing(DriveApp.Access.PRIVATE, DriveApp.Permission.NONE);
  } catch (_e) { log_('getOrCreateRootFolder', 'Error setting sharing: ' + (_e.message || _e)); }
...
function setupDriveFolderForGrievance(grievanceId) {
  try {
    ...
    if (!folderParent) {
      ...
      var membersRoot   = null;
      if (membersRootId) {
        try { membersRoot = DriveApp.getFolderById(membersRootId); } catch (_e) { log_('setupDriveFolderForGrievance', 'Error: ' + (_e.message || _e)); }
      }
      if (!membersRoot) {
  ...
  try {
    ...
    try {
      ...
      if (msg.indexOf('quota') !== -1 || msg.indexOf('limit') !== -1 || msg.indexOf('rate') !== -1) {
        log_('createMeetingCalendarEvent', 'Calendar quota exceeded, skipping event creation: ' + msg);
      } else {
        log_('createMeetingCalendarEvent', 'Error creating calendar event: ' + msg);
      }
      return '';
    }
    ...
    log_('createMeetingCalendarEvent', 'Error: ' + error.message);
    return '';
  }
}
/**
...
function emailMeetingAttendanceReport(meetingId, recipientEmails) {
  ...
  try {
    ...
    var dateStr = meetingDate instanceof Date ? Utilities.formatDate(meetingDate, Session.getScriptTimeZone(), 'MM/dd/yyyy') : String(meetingDate);
    ...
    var validatedRecipients = emails.map(function(e) { return e.trim(); }).join(',');
    ...
    return { success: true, message: 'Attendance report emailed to ' + validatedRecipients };
  } catch (error) {
    log_('emailMeetingAttendanceReport', 'Error: ' + error.message);
...
function createMeetingDocs(meetingData) {
  ...
  try {
    ...
    log_('createMeetingDocs', 'Error: ' + error.message);
...
function setDocViewOnlyByLink(docUrl) {
  try {
    ...
    try {
      ...
      log_('setDocViewOnlyByLink', 'Domain sharing unavailable, falling back to ANYONE_WITH_LINK: ' + domainError.message);
      file.setSharing(DriveApp.Access.ANYONE_WITH_LINK, DriveApp.Permission.VIEW);
    }
  } catch (error) {
    log_('setDocViewOnlyByLink', 'Error: ' + error.message);
  }
}
...
function emailMeetingDocLink(meetingName, meetingDate, docUrl, docType, recipientEmails) {
  ...
  try {
    ...
    log_('emailMeetingDocLink', 'Error: ' + error.message);
  }
}
...
function getAllStewardEmails_() {
  try {
    ...
    log_('getAllStewardEmails_', 'Error: ' + e.message);
    return '';
  }
}
...
function processMeetingDocNotifications() {
  ...
  try {
    ...
    log_('processMeetingDocNotifications', 'Error: ' + error.message);
  }
  return result;
}
...
function syncDeadlinesToCalendar() {
  try {
    ...
    log_('syncDeadlinesToCalendar', 'Error: ' + (error.message || error));
    return errorResponse(error.message);
  }
}
...
function sendDeadlineReminders(daysAhead) {
  try {
    ...
    log_('sendDeadlineReminders', 'Error: ' + (error.message || error));
    return errorResponse(error.message);
  }
}
...
function sendEmailToMember(memberId, subject, body) {
  try {
    ...
    log_('sendEmailToMember', 'Error: ' + (error.message || error));
    return errorResponse(error.message);
  }
}
...
function createSignatureReadyPDF(folder, data) {
  ...
  try {
    ...
    body.replaceText('{{Date}}', escapeReplacement_(Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'MM/dd/yyyy')));
    ...
    log_('createSignatureReadyPDF', 'Error: ' + e.message);
...
function onGrievanceFormSubmit(e) {
  ...
  try {
    ...
    log_('onGrievanceFormSubmit', 'Error: ' + err.message);
  }
}
...
// Dead code removed: _getBottomNavCSS_(), _getBottomNavHTML_() — legacy mobile nav helpers, zero callers
...
function getWebAppResourceLinks() {
  ...
  if (configSheet && configSheet.getLastRow() >= 3) {
    ...
    } catch (_e) { log_('getWebAppResourceLinks', 'Error: ' + (_e.message || _e)); }
  }
...
function syncConstantContactEngagement() {
  ...
  // Pre-count total matches for accurate progress reporting
  for (var ci = 0; ci < memberData.length; ci++) {
    var checkEmail = (col_(memberData[ci], MEMBER_COLS.EMAIL) || '').toString().toLowerCase().trim();
    if (checkEmail && emailToContactId[checkEmail]) totalMatches++;
  }
  ...
  for (var i = 0; i < memberData.length; i++) {
    ...
    if (contactId) {
      ...
      if (Date.now() - syncStart > 270000) { // 4.5 min safety margin
        log_('syncCCEngagement', 'Time limit approaching, processed ' + processedCount + ' of ' + totalMatches);
        timeLimitReached = true;
        // Preserve existing values for remaining members
      ...
      if (processedCount % 25 === 0) {
        ss.toast('Processing... ' + processedCount + '/' + totalMatches + ' contacts', '📧 CC Sync', 5);
      }
    } else {
  ...
  // Prevents data corruption when MEMBER_COLS constants are stale.
  if (openRateUpdates.length > 0) {
    var ccWriteCols = resolveColumnsByHeader_(memberSheet, [
      { key: 'OPEN_RATE',    header: 'Open Rate %',          fallback: MEMBER_COLS.OPEN_RATE },
      { key: 'CONTACT_DATE', header: 'Recent Contact Date',  fallback: MEMBER_COLS.RECENT_CONTACT_DATE }
    ]);
    memberSheet.getRange(2, ccWriteCols.OPEN_RATE, openRateUpdates.length, 1).setValues(openRateUpdates);
    memberSheet.getRange(2, ccWriteCols.CONTACT_DATE, contactDateUpdates.length, 1).setValues(contactDateUpdates);
  }

  // Step 5: Report results
  var summary = 'Constant Contact Sync Complete' + (timeLimitReached ? ' (partial)' : '') + '!\n\n' +
  ...
  if (timeLimitReached) {
    summary += '\n\n⚠️ Execution time limit approached — ' + processedCount + ' of ' +
      totalMatches + ' matched contacts were processed. Run again to continue.';
  }

  ui.alert('✅ CC Sync Complete', summary, ui.ButtonSet.OK);
  log_('CC sync', matchCount + ' matches out of ' + memberData.length + ' members' +
    (timeLimitReached ? ' (time limit, processed ' + processedCount + '/' + totalMatches + ')' : ''));
}
...
function MIGRATE_ADD_DISMISS_MODE_COLUMN() {
  ...
  var sheet = ss.getSheetByName(SHEETS.NOTIFICATIONS);
  var ui;
  try { ui = SpreadsheetApp.getUi(); } catch (_e) { log_('MIGRATE_ADD_DISMISS_MODE_COLUMN', 'Error getting UI: ' + (_e.message || _e)); }
...
function SETUP_DRIVE_FOLDERS() {
  ...
  var ui;
  try { ui = SpreadsheetApp.getUi(); } catch (_e) { log_('SETUP_DRIVE_FOLDERS', 'Error getting UI: ' + (_e.message || _e)); }
...
function generateGrievancePDF_(formData) {
  try {
    ...
    var valueMap = {
      ...
      'agency':     formData.agency     || getConfigValue_(CONFIG_COLS.ORG_NAME, 'Organization'),
    ...
    for (var id in valueMap) {
      if (!Object.prototype.hasOwnProperty.call(valueMap, id)) continue;
...
function submitGrievanceSignature(sigToken, sigBase64) {
  try {
    ...
    var pdfData = {
      ...
      agency:     getConfigValue_(CONFIG_COLS.ORG_NAME, 'Organization'),
      region:     regionVal,
      workloc:    workLoc,
      articles:   grievanceObj.articles,
      step:       grievanceObj.step,
...
function initiateGrievance(stewardEmail, data, idemKey) {
  try {
    ...
    setCol_(rowData, GRIEVANCE_COLS.CURRENT_STEP, data.step || 'Informal');
    ...
    var pdfData = {
      ...
      agency:     formOverrides.agency || getConfigValue_(CONFIG_COLS.ORG_NAME, 'Organization'),
...
function getGrievanceFormOptions() {
  try {
    ...
    for (var key in colMap) {
      if (!Object.prototype.hasOwnProperty.call(colMap, key) || !colMap[key]) continue;
...
function SETUP_CALENDAR() {
  ...
  var ui;
  try { ui = SpreadsheetApp.getUi(); } catch (_e) { log_('SETUP_CALENDAR', 'Error getting UI: ' + (_e.message || _e)); }
  var msg = result.success
