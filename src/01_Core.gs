...
var VERSION_INFO = (function() {
  var ver = (typeof COMMAND_CONFIG !== 'undefined' && COMMAND_CONFIG.VERSION) ? COMMAND_CONFIG.VERSION : '4.36.0';
  var parts = ver.split('.');
  return {
    version: ver,
    MAJOR: parts.length > 0 ? parseInt(parts[0], 10) : 4,
    MINOR: parts.length > 1 ? parseInt(parts[1], 10) : 36,
...
var CONFIG_HEADER_MAP_ = [
  ...
  // Modal Hub (v4.36.0) — set to 'yes' to show the centralized Modal Hub launcher
  { key: 'ENABLE_MODAL_HUB',        header: 'Enable Modal Hub' }
];
