...
// ============================================================================
// T1-5a: WorkloadVault_Backups folder security
// ============================================================================

describe('WorkloadVault backup folder security', () => {
  test('sets PRIVATE sharing on newly created backup folder', () => {
    jest.clearAllMocks();
    ...
    var vaultHeaders = new Array(25).fill('header');
    var vaultRow = new Array(25).fill('data');
    var sheet = createMockSheet(SHEETS.WORKLOAD_VAULT || 'Workload Vault', [vaultHeaders, vaultRow]);
    var ss = createMockSpreadsheet([sheet]);
    SpreadsheetApp.getActiveSpreadsheet.mockReturnValue(ss);

    // Call the backup function — may be exposed as wkCreateBackup or WorkloadService.createBackup
    if (typeof wkCreateBackup === 'function') {
      wkCreateBackup();
    } else if (typeof WorkloadService !== 'undefined' && typeof WorkloadService.createBackup === 'function') {
      WorkloadService.createBackup();
    }

    expect(mockFolder.setSharing).toHaveBeenCalledWith(
      DriveApp.Access.PRIVATE,
      DriveApp.Permission.NONE
    );
    expect(mockFolder.setDescription).toHaveBeenCalledWith(
